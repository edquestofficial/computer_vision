...
def process_registration_object(employee_id: str, name: str, photos: List[UploadFile]) -> dict:
    """
    THIS FUNCTION DOES ALL THE WORK.
    It takes the object from FastAPI, loops through images,
    saves them, and creates embeddings for each one.
    """
    print(f"Vector_store processing ID: {employee_id}, Name: {name}")
    ...
    for i, photo in enumerate(photos):
        ...
        temp_path = os.path.join(TEMP_DIR, f"{employee_id}_{i}_{photo.filename}")
        ...
        try:
            ...
            results.append({"status": "error", "message": f"Failed to save file: {e}", "image_index": i})
            continue # Skip to the next photo
            
        # 2. Call the internal embedding function
        # This is your "FIASS -> inser" for each image
        result = _create_embedding_for_file(
...
def _create_embedding_for_file(employee_id: str, name: str, image_path: str, image_index: int) -> dict:
    """
